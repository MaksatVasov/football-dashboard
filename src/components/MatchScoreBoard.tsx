...
const formatMinute = (e: FixtureEvent) => `${e.time.elapsed}'${e.time.extra ? `+${e.time.extra}` : ""}`;

export default function MatchScoreboard({ match }: { match: FixtureDetails }) {
  const { teams, goals, fixture } = match;

  const goalsOf = (teamId: number) => (match.events ?? []).filter((e) => e.type === "Goal" && e.detail !== "Missed Penalty" && e.team.id === teamId);
  ...
  return (
    <div className="relative w-full bg-surface border border-line rounded-2xl overflow-hidden py-8 px-4 md:px-10 flex items-start justify-between shadow-sm">
      ...
      <div className="relative z-10 flex flex-col gap-3 w-1/3">
        <div className="flex items-center gap-3">
          ...
          <span className="text-fg text-[15px] md:text-xl font-bold hidden sm:block">{teams.home.name}</span>
        </div>
        <div className="flex flex-col text-[11px] md:text-[13px] text-muted gap-1 mt-2">
          {goalsOf(teams.home.id).map(renderGoal)}
        </div>
      </div>

      <div className="relative z-10 flex flex-col items-center w-1/3">
        <span className="text-fg font-medium text-[12px] md:text-[13px] mb-3 text-center">
          {fixture.venue.name ?? "Venue TBC"}
        </span>
        <div className="flex items-center justify-center gap-4 md:gap-6">
          <span className="text-fg text-3xl md:text-4xl font-bold">{goals.home ?? "-"}</span>
          <span className="text-muted text-sm font-semibold">{fixture.status.short}</span>
          <span className="text-fg text-3xl md:text-4xl font-bold">{goals.away ?? "-"}</span>
        </div>
      </div>

      <div className="relative z-10 flex flex-col gap-3 w-1/3 items-end">
        <div className="flex items-center gap-3">
          <span className="text-fg text-[15px] md:text-xl font-bold hidden sm:block">{teams.away.name}</span>
          <img src={teams.away.logo} alt={teams.away.name} className="w-8 h-8 object-contain" />
        </div>
        <div className="flex flex-col text-[11px] md:text-[13px] text-muted gap-1 items-end text-right mt-2">
