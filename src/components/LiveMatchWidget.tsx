...
function WidgetLayout({ isLoadingDetails, awayTeam, homeTeam, homeStats, awayStats, WIDGET_STATS }: { isLoadingDetails: boolean, awayTeam: TeamStats, homeTeam: TeamStats, homeStats: Stats, awayStats: Stats, WIDGET_STATS: WidgetType[] }) {
...
export default function LiveMatchWidget() {
  ...
  const { homeStats, awayStats, WIDGET_STATS, homeTeam, awayTeam, isDataReal, isLoadingDetails } = useLiveMatchWidget();

  if (!isDataReal) return (
    <div className="w-full lg:grow lg:basis-0 flex flex-col items-center justify-center min-h-105 bg-[rgba(0,0,0,0.01)] rounded-3xl border border-gray-100/80 shadow-sm p-6 text-center">
  ...
  return (
    <div id="liveWidget" className="w-full lg:grow lg:basis-0 bg-[rgba(0,0,0,0.01)] rounded-3xl shadow-sm border border-gray-100/80 flex flex-col items-center p-2.5">
      <WidgetLayout  isLoadingDetails={isLoadingDetails} awayTeam={awayTeam} WIDGET_STATS={WIDGET_STATS} awayStats={awayStats} homeTeam={homeTeam} homeStats={homeStats}/>
    </div>
  );
}