...
export function Shopping() {
  ...
  return (
    <div id="shop" className="w-full pt-8 mb-12 border-line border-t-[3px]">
      <div className="flex items-center gap-2 mb-4 px-2">
        <span className="text-lg">👕</span>
        <h2 className="text-[15px] font-bold text-fg">Shopping</h2>
      </div>
      ...
      <div onScroll={checkScroll} ref={div} className="flex overflow-x-auto gap-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] scrollbar-none px-2 pb-2">
        {SHOP_ITEMS.map((item) => (
          <Link to={`/shop/${item.id}`} key={item.id} className="flex flex-col gap-3 shrink-0 w-45 cursor-pointer group">
            <div className="w-full h-50 bg-surface-2 rounded-2xl flex items-center justify-center p-4 transition-colors group-hover:bg-surface">
              <img src={item.img} alt={item.name} className="w-full h-full object-contain drop-shadow-sm" />
            </div>
            <div className="flex flex-col px-1">
              <span className="text-[13px] font-semibold text-fg mb-1">{item.name}</span>
              <span className="text-[13px] font-bold text-accent">{item.price}</span>
            </div>
          </Link>
        ))}
