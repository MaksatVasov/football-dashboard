...
function PlayerDot({ p, extra, colors, }: { p: PitchPlayer; extra?: PlayerExtra | undefined; colors: string; }) {
  ...
  return (
    ...
    >
      ...
      <span
        className={`pointer-events-none invisible opacity-0 group-hover:visible group-hover:opacity-100 group-focus:visible group-focus:opacity-100 transition-opacity absolute z-30 flex w-max max-w-56 items-center gap-2 rounded-xl border border-gray-100 bg-white p-2 text-left shadow-xl ${vertical} ${horizontal}`}
      >
        {extra?.photo && (
          <img
            src={extra.photo}
        ...
        <span className="flex min-w-0 flex-col">
          <span className="truncate text-xs font-bold text-gray-900">{p.name}</span>
          <span className="text-[11px] font-normal text-gray-500">
            #{p.number} · {POS_LABEL[p.pos] ?? p.pos}
          </span>
          {Number.isFinite(rating) && (
            <span className="text-[11px] font-bold text-[#5942AA]">
...
export default function TeamFormation({ match }: { match: FixtureDetails }) {
  ...
  if (!home || !away) {
    return (
      <div className="bg-white mt-6 rounded-2xl p-6 w-full border border-gray-100 shadow-sm text-center text-gray-500">
      ...
      <div className="flex items-center justify-between mb-6 px-2">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="text-gray-900 font-bold">{teams.home.name}</span>
            <span className="text-gray-500 text-sm">{home.formation ?? "-"}</span>
          </div>
        </div>
        <span className="text-gray-400 font-bold">{fixture.status.short}</span>
        <div className="flex items-center gap-3">
          <div className="flex flex-col items-end">
            <span className="text-gray-900 font-bold">{teams.away.name}</span>
            <span className="text-gray-500 text-sm">{away.formation ?? "-"}</span>
          </div>
